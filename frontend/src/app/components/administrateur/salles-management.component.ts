  ...
  template: `
    <section class="rooms-shell">
      ...
      <div class="filters panel">
        <label>Centre
          <select [(ngModel)]="filterCentreId" (change)="onFilterCentreChange()">
        ...
        <label>Spécialité
          <select [(ngModel)]="filterSpecialiteId">
      ...
      <div class="panel" *ngIf="showForm">
        ...
        <div class="form-grid">
          ...
          <label>Centre *
            <select [(ngModel)]="form.centreId" (change)="onFormCentreChange()">
...
export class SallesManagementComponent implements OnInit {
  ...
  get filteredSalles(): Salle[] {
    return this.salles.filter(salle =>
      (!this.filterCentreId || salle.centre.id === this.filterCentreId) &&
      (!this.filterSpecialiteId || salle.specialite.id === this.filterSpecialiteId));
  }
  ...
  onFilterCentreChange(): void { this.filterSpecialiteId = undefined; this.loadSpecialitesForCentre(this.filterCentreId, list => this.filterSpecialites = list); }
  ...
  startCreate(): void { this.editingSalle = undefined; this.form = this.emptyForm(); this.formSpecialites = []; this.formError = ''; this.showForm = true; }
  ...
  saveSalle(): void {
    ...
    this.saving = true;
    const request = this.editingSalle?.id ? this.api.updateSalle(this.editingSalle.id, payload) : this.api.createSalle(payload);
  ...
  removeSalle(salle: Salle): void {
    ...
    this.api.deleteSalle(salle.id).subscribe({ next: () => this.loadData(), error: () => window.alert('Impossible de supprimer cette salle.') });
  }
  ...
  private loadSpecialitesForCentre(centreId: number | undefined, apply: (list: Specialite[]) => void): void {
    ...
    this.api.getCentreSpecialitesByCentre(centreId).subscribe({ next: response => apply((response.data || []).map(item => item.specialite).filter((item): item is Specialite => !!item)), error: () => apply([]) });
  }
